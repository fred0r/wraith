...
bool UpdateModule::bupdating_ = false;
bool UpdateModule::updated_ = false;
int UpdateModule::cnt_ = 0;

bool UpdateModule::is_bupdating() { return bupdating_; }
void UpdateModule::set_bupdating(bool value) { bupdating_ = value; }
bool UpdateModule::is_updated() { return updated_; }
void UpdateModule::set_updated(bool value) { updated_ = value; }
int UpdateModule::tick() { return ++cnt_; }
void UpdateModule::reset_tick() { cnt_ = 0; }

#define STREAM_CHUNKS_PER_TICK 64
...
static bool is_encrypted_link(int idx)
{
  ...
  /* 1.4.x bots: legacy AES-256-ECB botlink is the encrypted type */
  return (type == LINK_GHOSTCASE3);
}
...
{
  if (dcc[idx].status & STAT_GETTINGU || UpdateModule::is_updated()) 
    return;
...
static void update_ufsend(int idx, char *par)
{
  ...
  if (dcc_total == max_dcc) {
    ...
    if (sock < 0 || (open_telnet_return = open_telnet_dcc(sock, ip, port)) < 0) {
      ...
      dcc[i].u.xfer->kind = XferKind::Binary;
...
{
  if (!par || !*par || UpdateModule::is_updated()) 
    return;
...
static void stream_continue(void *)
{
  ...
  while (stream_state.stream.tell() < stream_state.stream.length() &&
         ...
         !sock_has_data(SOCK_DATA_OUTGOING, dcc[idx].sock)) {
...
static void start_sending_binary(int idx, bool streamable)
{
  ...
  if (UpdateModule::is_bupdating()) return;
  UpdateModule::set_bupdating(true);
...
//    simple_snprintf(update_fpath, sizeof update_fpath, "%s/%s", bindir, update_file);
  ...
  if (streamable) {
    ...
    if ((res = raw_dcc_send(update_fpath, "*binary", "(binary)", &j)) != DccSendResult::Ok) {
      putlog(LOG_BOTS, "*", "%s -- can't send new binary",
             ...
             res == DccSendResult::BadFn  ? "BAD FILE" :
             res == DccSendResult::FEmpty ? "EMPTY FILE" : "UNKNOWN REASON!");
      dcc[idx].status &= ~(STAT_SENDINGU);
      dcc[idx].status |= STAT_UPDATED;
      {
...
static void check_updates()
{
  if (isupdatehub()) {
    ...
    int cnt = UpdateModule::tick();
    if ((cnt > 1) && UpdateModule::is_bupdating())  UpdateModule::set_bupdating(false);
    if (UpdateModule::is_bupdating()) return;
    UpdateModule::reset_tick();
