...
std::string CtcpModule::cloak_bxver_;
std::string CtcpModule::cloak_os_;
std::string CtcpModule::cloak_osver_;
std::string CtcpModule::cloak_host_;
char ctcpversion[300] = "";
char ctcpversion2[300] = "";
char ctcpuserinfo[200] = "";
char autoaway[100] = "";
std::string CtcpModule::kickprefix_;
std::string CtcpModule::bankickprefix_;

const char *CtcpModule::kickprefix() { return kickprefix_.c_str(); }
const char *CtcpModule::bankickprefix() { return bankickprefix_.c_str(); }
...
void CtcpModule::script_changed()
{
  /* Check if this was called from init_vars() before ctcp_init() is called */
  if (cloak_bxver_.empty())
    return;

  char tmp[200] = "", *p = NULL;

  ctcpversion[0] = ctcpversion2[0] = ctcpuserinfo[0] = autoaway[0] = 0;
  kickprefix_.clear();
  bankickprefix_.clear();
  
  switch (cloak_script) {
  case CLOAK_PLAIN:
    simple_snprintf(ctcpversion, sizeof(ctcpversion), "\002BitchX-%s\002 by panasync - %s %s : \002Keep it to yourself!\002", cloak_bxver_.c_str(), cloak_os_.c_str(), cloak_osver_.c_str());
    ctcpuserinfo[0] = 0;
    kickprefix_.clear();
    bankickprefix_.clear();
  ...
  case CLOAK_CRACKROCK:
    simple_snprintf(ctcpversion, sizeof(ctcpversion), "BitchX-%s\002/\002%s %s:(\002c\002)\037rackrock\037/\002b\002X \037[\0373.0.1�9\037]\037 :\002 Keep it to yourself!\002", cloak_bxver_.c_str(), cloak_os_.c_str(), cloak_osver_.c_str());
    strlcpy(ctcpuserinfo, "crack addict, help me.", sizeof(ctcpuserinfo));
    strlcpy(autoaway, "automatically dead", sizeof(autoaway));
    kickprefix_ = "\002c\002/\037k\037: ";
    bankickprefix_ = "\002c\002/\037kb\037: ";
    break;
  case CLOAK_NEONAPPLE:
    simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), cloak_osver_.c_str());
    strtolower(tmp);
    simple_snprintf(ctcpversion, sizeof(ctcpversion), "bitchx-%s\037(\037%s\037):\037 \002n\002eon\037a\037ppl\002e\002\037/\037\002v\0020\037.\03714i : \002d\002ont you wish you had it\037?\037", cloak_bxver_.c_str(), tmp);
    ...
    kickprefix_ = "\037[na\002(\037k\037)\002]\037 ";
    bankickprefix_.clear();
    break;
  case CLOAK_TUNNELVISION:
    strlcpy(tmp, cloak_bxver_.c_str(), sizeof(tmp));
    p = tmp;
    p += strlen(tmp) - 1;
    p[1] = p[0];
    p[0] = '\037';
    ...
    break;
  case CLOAK_ARGON:
    simple_snprintf(ctcpversion, sizeof(ctcpversion), ".\037.(\037argon\002/\0021g\037)\037 \002:\002bitchx-%s", cloak_bxver_.c_str());
    ctcpuserinfo[0] = 0;
    strlcpy(autoaway, "\037(\037ar\037)\037 auto-away \037(\03710m\037)\037", sizeof(autoaway));
    kickprefix_ = "\037(\037ar\037)\037 ";
    bankickprefix_ = "\037(\037ar\037)\037 ";
    break;
  case CLOAK_EVOLVER:
    simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037evolver\037(\00202x9\002)\037: bitchx\037(\002%s\002) \037�\037 %s\002/\002%s : eye yam pheerable now!", cloak_bxver_.c_str(), cloak_os_.c_str(), cloak_osver_.c_str());
    ctcpuserinfo[0] = 0;
    strlcpy(autoaway, "[\037\002i\002dle for \037[\03710 minutes\037]]", sizeof(autoaway));
    kickprefix_ = "\037ev\002!\002k\037 ";
    bankickprefix_ = "\037ev\002!\002bk\037 ";
    break;
  case CLOAK_PREVAIL:
    simple_snprintf(ctcpversion, sizeof(ctcpversion), "%s\037!\037%s bitchx-%s \002-\002 prevail\037[\0370123\037]\037 :down with people", cloak_os_.c_str(), cloak_osver_.c_str(), cloak_bxver_.c_str());
    ...
    kickprefix_ = "\037[\037pv\037!\037k\037]\037 ";
    bankickprefix_ = "\037[\037pv\037!\037bk\037]\037 ";
    break;
  case CLOAK_MIRC:
  {
    char mircver[5] = "";
  ...
  case CLOAK_CYPRESS:
  {
    ...
    switch (randint(16)) {
    case 0:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "bitchx\037-\037%s \037/\037 cypress\037.\03701i%s", cloak_bxver_.c_str(), theme);
      break;
    case 1:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "cypress\037.\03701i%s \037/\037 bitchx\037-\037%s", theme, cloak_bxver_.c_str());
      break;
    case 2:
      simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), cloak_osver_.c_str());
      strtolower(tmp);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "cypress\037.\03701i%s %s\037(\037%s\037)\037 bitchx\037-\037%s)", theme, tmp, cloak_host_.c_str(), cloak_bxver_.c_str());
      break;
    case 3:
      simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), cloak_osver_.c_str());
      strtolower(tmp);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "bitchx\037-\037%s %s\037(\037%s\037)\037 cypress\037.\03701i%s", cloak_bxver_.c_str(), tmp, cloak_host_.c_str(), theme);
      break;
    case 4:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "%s\002/\002%s: BitchX-%s \002[\002cypress\002]\002 v01i%s", cloak_os_.c_str(), cloak_osver_.c_str(), cloak_bxver_.c_str(), theme);
      break;
    case 5:
      p = replace(cloak_osver_.c_str(), ".", "\037.\037");
      simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), p);
      strtolower(tmp);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037.\037.cypress\037.\03701i%s %s\037(\037%s\037)\037 bitchx\037/\037%s",theme, tmp, cloak_host_.c_str(), cloak_bxver_.c_str());
      break;
    case 6:
      p = replace(cloak_osver_.c_str(), ".", "\037.\037");
      simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), p);
      strtolower(tmp);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "cypress\002.\00201i%s\037(\037bitchx\002.\002%s\037)\037\002.\002. %s\037(\037%s\037)\037",theme, cloak_bxver_.c_str(), tmp, cloak_host_.c_str());
      break;
    case 7:
      p = replace(cloak_osver_.c_str(), ".", "\037.\037");
      simple_snprintf(tmp, sizeof(tmp), "%s %s", cloak_os_.c_str(), p);
      strtolower(tmp);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037.\037.cypress\037.\03701i%s - bitchx\037.\037%s\002/\002%s", theme, cloak_bxver_.c_str(), tmp);
      break;
    case 8:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\002BitchX-%s\002 by panasync \002-\002 %s %s", cloak_bxver_.c_str(), cloak_os_.c_str(), cloak_osver_.c_str());
      break;
    case 9:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037[\037cypress\002/\00201i\037]\037 - %s \037[\037bx\002/\002%s\037]\037", theme, cloak_bxver_.c_str());
      break;
    case 10:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037[\037\002b\002itchx\002.\002%s\037]\037 \002+\002 \037[\037cypress\002.\00201i\037]\037 %s",cloak_bxver_.c_str(), theme);
      break;
    case 11:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037[\037BitchX\002/\002%s\037(\037cypress\002/\00201i\037)]\037 %s", cloak_bxver_.c_str(), theme);
      break;
    case 12:
      lowercase(cloak_os_);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "bitchx\037/\037%s %s %s \037(\037cypress\037/\03701i\037)\037 %s", cloak_bxver_.c_str(), cloak_os_.c_str(), cloak_osver_.c_str(), theme);
      break;
    case 13:
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "\037.\037.cypress\037/\03701i\037!\037bitchx\037/\037%s\037.\037.%s", cloak_bxver_.c_str(), theme);
      break;
    case 14:
      lowercase(cloak_bxver_);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "cypress\002\037.\037\002\037.\03701i\002/\002bitchx\037.\037\002\037.\037\002%s%s", cloak_bxver_.c_str(), theme);
      break;
    case 15:
      lowercase(cloak_bxver_);
      simple_snprintf(ctcpversion, sizeof(ctcpversion), "cypress\037.\03701i\037/\037bx%s \037(\037%s\037)\037", cloak_bxver_.c_str(), theme);
      break;
    }
    ctcpuserinfo[0] = 0;
    strlcpy(autoaway, "autoaway after 40 min", sizeof(autoaway));
    kickprefix_ = "\002.\002.\037(\037\002c\002yp\002/\002k\037)\037 ";
    bankickprefix_ = "\002.\002.\037(\037\002c\002yp\002/\002bk\037)\037 ";
    break;
  }
  }
}
...
void CtcpModule::init()
{
  struct utsname un;

  bzero(&un, sizeof(un));
  if (!uname(&un)) {
    cloak_os_ = un.sysname;
    cloak_osver_ = un.release;
    cloak_host_ = un.nodename;
  } else {
    /* shit, we have to come up with something ourselves.. */
    switch (randint(2)) {
    case 0:
      cloak_os_ = "Linux";
      cloak_osver_ = "2.6.25.5";
      break;
    case 1:
      cloak_os_ = "FreeBSD";
      cloak_osver_ = "7.0-p4";
  ...
  size_t dot = cloak_host_.find('.');
  if (dot != std::string::npos)
    cloak_host_.resize(dot);

  switch (randint(4)) {
  case 0:
    cloak_bxver_ = "1.1-final";
    break;
  case 1:
    cloak_bxver_ = "1.0c18";
    break;
