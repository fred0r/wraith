...
int timer_get_shortest(egg_timeval_t *howlong)
{
	...
	if (!timer_repeat_list.empty())
		best = timer_repeat_list.front().get();

	if (!timer_once_list.empty()) {
		Timer *once = timer_once_list.front().get();
		if (!best ||
		    once->trigger_time().sec < best->trigger_time().sec ||
		    (once->trigger_time().sec == best->trigger_time().sec &&
		     once->trigger_time().usec < best->trigger_time().usec))
			best = once;
	}
	...
	timer_diff(&now, (egg_timeval_t*)&best->trigger_time(), howlong);
	return(0);
}
