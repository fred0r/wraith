...
In a source tree or source tarball the root `Makefile` is generated by
`./configure`, so run that before any `make` target (including `make clean`):

```
./configure && gmake     # FreeBSD/OpenBSD/NetBSD (GNU make)
./configure && make      # Linux
```

`build.sh` builds and packages a binary. It requires a `pack.cfg`, which is
embedded into the binary, so create one with the shipped generator first:

```
./scripts/mkpackcfg/mkpackcfg.py -n mypack -p <binarypass> -o <owner> <pass> -u <hub> <host> <port> > mypack.cfg
./build.sh mypack.cfg
./build.sh -d mypack.cfg     # debug build
```
